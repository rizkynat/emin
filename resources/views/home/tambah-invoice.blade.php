...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>Tambah Invoice | Emin</title>
...
@section('content')
  ...
  <div
        ...
        <h4
            ...
            >
            <div class="bg-primary-normal w-60 h-8 shadow-md rounded-r-3xl"><span class="ml-4 text-primary-white">Tambah Invoice</span></div>
        </h4>
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              ...
              @endif
                <form action="/tambah-invoice/" method="post">
                    @csrf                    
                    <label class="block text-sm mt-4">
                        <span class="text-gray-700 dark:text-gray-400">Judul Artikel</span>
                        <select name="id_artikel" class="scrollbar block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-green dark:focus:shadow-outline-gray" multiple="">
                        @foreach ($artikels as $artikel)
                            <option value="{{$artikel->id_artikel}}">{{$artikel->judul_artikel}}</option>
                        @endforeach
                        </select>
                    </label>
