...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>Tambah Status Artikel | Emin</title>
...
@section('content')
  ...
  <div
        ...
        <h4
            ...
            >
            <div class="bg-primary-normal w-60 h-8 shadow-md rounded-r-3xl"><span class="ml-4 text-primary-white">Tambah Status Artikel</span></div>
        </h4>
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              ...
              @endif
                <form action="/tambah-artstatus/{{$id_artikel}}" method="post">
                    @csrf
                    <label class="block text-sm mt-4">
                        <span class="text-gray-700 dark:text-gray-400">Judul Artikel</span>
                        <input readonly type="text" value="{{$artikel[0]->judul_artikel}}" name="id_artikel" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-primary-hover focus:outline-none focus:shadow-outline-green dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="Vol. 7 No.1">
                    </label>
                    
                    <label class="block text-sm mt-4">
                        <span class="text-gray-700 dark:text-gray-400">Keterangan Status</span>
                        <select name="kode_status" class="scrollbar block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-green dark:focus:shadow-outline-gray" multiple="">
                        @foreach ($arrayTambahStatuss as $arrayTambahStatus)
                        @php($arr = array('ir','sa','rd','ii','plri', 'wp'))
                            @if(!in_array($arrayTambahStatus->kode_status,$arr))
                            <option value="{{$arrayTambahStatus->kode_status}}">{{$arrayTambahStatus->keterangan_status}}</option>
                            @endif
                        @endforeach
                        </select>
                    </label>
                    ...
                    <div class="mt-4">
                    <a href="/tambah-artstatus/{{$id_artikel}}">
